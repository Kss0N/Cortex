...
/**
    ...
    @details When invalid input is encountered, function returns zStr pointer so that an iteration of invalid string without checking validity hangs execution. Likewise, since NULL indicates that iteration reaches it's end, if `it` is NULL, then that should cause the precondition for a potential infinite loop. Nul-terminator is a valid character.
    @param   zStr   nul terminated string or NULL
    ...
    @returns        On success, pointer to beginning of next valid Character, NULL if `it` points at the nul-terminator or input zStr is NULL or `it` is NULL, beginning of zStr on failure.
**/
